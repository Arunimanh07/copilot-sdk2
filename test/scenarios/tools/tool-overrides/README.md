...
The flag is set per-tool in TypeScript (`overridesBuiltInTool: true`), Python (`overrides_built_in_tool=True`), and Go (`OverridesBuiltInTool: true`). In C#, built-in tool overrides are declared on the session config via `BuiltInToolOverrides`.
